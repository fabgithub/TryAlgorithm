...
int palindrome(const std::string &s)
{
    int nRes = 0;
    ...
    // 查找奇数个字符的个数
    int nOddCount = 0;
    CharCount::iterator iterCC = cc.begin();
    for (; cc.end() != iterCC; ++iterCC)
    {
        if (iterCC->second % 2)
        {
            ++nOddCount;
        ...
        iterCC->second /= 2;
//        std::cout << iterCC->first << "(" << iterCC->second << "), ";
    }
    std::cout << std::endl;
    if ((s.size() % 2 == 0 && nOddCount > 0) || (s.size() % 2 == 1 && nOddCount != 1) )
    {
        return 0;
    }
    // 计算
    JxBigNum result = JieCheng((int) s.size() / 2);
    for (iterCC = cc.begin(); cc.end() != iterCC; ++iterCC)
    {
//        std::cout << result << ", ";
        result = result / JieCheng(iterCC->second);
    }
//    std::cout << result;
//    std::cout << std::endl;
    result %= 1000000007;
    nRes = result.mNums.front();
    return nRes;
}
